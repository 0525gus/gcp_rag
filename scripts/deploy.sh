...
gcloud run deploy rag-mcp \
  ...
  --region="${REGION}" \
  --no-allow-unauthenticated \
  --set-env-vars="^|^GCP_PROJECT_ID=${PROJECT_ID}|GCP_REGION=${REGION}|RAG_CORPUS_NAME=${RAG_CORPUS_NAME}|GCS_RAW_BUCKET=${GCS_RAW_BUCKET:-unused}|GCS_NORMALIZED_BUCKET=${GCS_NORMALIZED_BUCKET:-unused}|FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-doc-state}|FIRESTORE_COLLECTION=${FIRESTORE_COLLECTION:-doc_state}|MCP_API_KEY=${MCP_API_KEY}|TOP_K_DEFAULT=${TOP_K_DEFAULT:-5}|SEARCH_FETCH_MULTIPLIER=${SEARCH_FETCH_MULTIPLIER:-3}|SEARCH_FETCH_MAX=${SEARCH_FETCH_MAX:-60}" \
