...
2026-09-10: RAG-001, CFG-001, ASYNC-001의 로컬 코드 수정과 회귀 검증을 진행했다.
현재 동작, 배포 순서와 외부 호출 중단 시 제한은
[안전성 수정 기록](SAFETY_FIXES_2026-09-10.md)을 따른다. 운영 배포·종단 검증 전이므로
아래 완료 상태는 아직 `DONE`으로 바꾸지 않는다.
...
|---|---|---|---|---|---|
| RAG-001 | P0 | IN PROGRESS | RAG 삭제 실패를 fail-closed로 변경 | 미지정 | 미지정 |
| SEC-001 | P0 | TODO | 학과 설정의 Cloud Build 업로드 차단 및 키 회전 | 미지정 | 미지정 |
| OPS-001 | P0 | TODO | Workflow의 실패·무실행 정상 처리 제거 | 미지정 | 미지정 |
| CFG-001 | P1 | IN PROGRESS | 학과 라우팅 설정을 fail-closed로 변경 | 미지정 | 미지정 |
| ASYNC-001 | P1 | IN PROGRESS | Cloud Tasks 멱등성 및 문서 버전 보호 | 미지정 | 미지정 |
