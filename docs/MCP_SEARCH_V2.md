...
- `currentDate`는 서버가 응답 시 한국 시간(UTC+09:00)으로 계산한 오늘 날짜다.
  `timeZone`은 `Asia/Seoul`이다. 위 날짜는 예시이며 서버가 매 요청마다 계산한다.
  빈 결과에도 포함하고, 캐시 적중 시에도 날짜를 갱신한다. 문서의 작성·개정일이나
  검색 근거가 아니며 날짜·기간·마감 비교에만 사용한다. 검색하지 않은 대화에는
  이 응답으로 날짜를 전달할 수 없다. 기존 v2에 필드를 추가하며 문서 구조는 유지한다.
