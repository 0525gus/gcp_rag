"""품질 게이트 — 관측용(기본) / 거부·폴백은 설정으로 전환 가능.
  ...
  G1 밀도     text_length / source_bytes
  G2 표 손실  문서 구조상 표 N개 중 마크다운에 M개만 남음
  EMPTY_TEXT  본문이 사실상 비었음

구 G2(표 '셀' 추출 실패율)와 G3(이미지 면적비)는 제거했다 — evaluate_quality
독스트링 참고.

**G2 는 두 값이 같은 집합을 세야만 성립한다.** `table_count` 는 렌더러가 표로
찍는 것과 같은 기준(rhwp 기준 `scope="body", recurse=False`)이어야 한다.
머리말/꼬리말 표나 중첩 표까지 세면 기준값이 부풀어 멀쩡한 문서가 손실 판정을
받는다 — 실제로 그랬고, `QG_MODE=log` 라 오탐이 로그에만 쌓여 오래 안 보였다
(docs/PARSER_BENCH.md Ⅶ).

G3 이 없다는 점은 아직 대가를 치르고 있다. G3 은 "밀도가 낮은 게 이미지가
많아서인지, 파싱이 실패해서인지"를 가르는 게이트였다. 그 구분을 G1 혼자 지느라
이미지 많은 공문의 오탐을 막으려 G1 임계값을 0.0005 까지 낮췄고(shared/config.py),
그만큼 **진짜 파싱 실패를 잡을 여력도 같이 잃었다.**
"""
