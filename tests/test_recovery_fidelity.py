...
def test_first_time_dlq_records_the_link_for_later_retry(
    ...
    monkeypatch.setattr(sync_main, "_ingest_file_copy", _boom)
...
def _wire_delete(
    ...
    monkeypatch.setattr(
        sync_main, "RagEngineClient", lambda: type("R", (), {"delete_by_file_id": lambda _s, _f: True})()
    )
    return gcs, store
...
def test_delete_surfaces_gcs_failure_instead_of_swallowing(
    ...
    _, store = _wire_delete(monkeypatch, {"norm": ["normalized/f1.md"], "raw": []})

    def _boom(*_a: Any, **_k: Any) -> int:
        raise RuntimeError("permission denied")

    monkeypatch.setattr(sync_main, "_delete_gcs_prefix_for_file", _boom)
