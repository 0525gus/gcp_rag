...
def test_size_limit_matches_rag_engine_per_file_type(ext: str, limit: int) -> None:
    assert sync_main._effective_limit(_Settings(), ext) == limit
...
def test_operator_limit_can_only_tighten_never_loosen() -> None:
    ...
    assert sync_main._effective_limit(_Tight(), ".pdf") == 5 * MB
    assert sync_main._effective_limit(_Settings(), ".pdf") == 50 * MB
...
def test_search_still_returns_top_k_after_filtering_hidden_docs(
    ...
    class _Rag:
        ...
        def retrieve(self, _q: str, *, top_k: int, **_kw: Any) -> list[SearchHit]:
            return hits[:top_k]
