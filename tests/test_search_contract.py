...
for key, value in {
    "GCP_PROJECT_ID": "test-project", "GCS_HWP_ORIGINAL_BUCKET": "raw",
    "GCS_SOURCE_BUCKET": "norm", "RAG_CORPUS_NAME": "projects/p/locations/l/ragCorpora/c",
}.items():
...
def server(monkeypatch):
    ...
    class Store:
        def get(self, fid):
            return DocState(file_id=fid, drive_id="d", name=f"{fid}.txt",
...
def test_protocol_exposes_only_search_and_returns_evidence_once(server):
    ...
    with TestClient(app.build_app()) as client:
        ...
        result = rpc("tools/call", {"name": "search", "arguments": {"query": "규정", "top_k": 5}})["result"]
        ...
        assert "context" not in payload and "coverage" not in payload and "chunk_count" not in payload
...
def test_empty_search_and_cached_response_do_not_claim_answerability(server):
    ...
    assert response == {"schemaVersion": 2, "documents": [], "documentCount": 0, "chunkCount": 0}
...
def test_html_preserves_scholarship_table_relationships():
    ...
    assert all(noise not in result for noise in ("<", "secret", "WEB-INF", "bad()", "숨김", "noise"))


def test_html_partial_and_escaped_chunks_do_not_lose_visible_text():
    assert "5이상" in clean_html_evidence('5이상&lt;br/&gt;&lt;span style="color:red"&gt;전공 12학점&lt;/span&gt;')
...
def test_long_spanned_conditions_are_kept_once_with_explicit_cell_references():
    ...
    result = html_to_text(f'<table><tr><td>A</td><td rowspan="2">{condition}</td></tr>'
                          '<tr><td>B</td></tr></table>')
...
def test_legacy_html_is_cleaned_but_code_examples_in_other_documents_are_preserved(server, monkeypatch):
    app, hits, _ = server
    hits[:] = [SearchHit('<h2>장학금</h2><p>15학점</p>', 0.1, SearchSource("f1")),
               SearchHit('HTML 예제: <div>code</div>', 0.2, SearchSource("f2"))]

    class Store:
        def get(self, fid):
            return DocState(file_id=fid, drive_id="d", name="content.html" if fid == "f1" else "code.txt",
                            status=DocStatus.INDEXED)
...
def test_html_is_cleaned_before_ingestion_hash_and_upload():
    ...
    result = sync._ingest_direct(body, _GateStore(), Gcs(),
                                _GateDrive('<h2>장학금</h2><p>15학점</p>'.encode()), _Settings())
...
def test_refresh_content_rebuilds_unchanged_html_preserving_modified_time(monkeypatch):
    ...
    existing = DocState(file_id="f1", drive_id="d", name="content.html", mime_type="text/html",
                        status=DocStatus.INDEXED, modified_time=modified, path="Drive/content.html")
    ...
    assert sync._ingest_with(body, store=store, gcs=_Gcs(), drive=Drive(), settings=_Settings())["status"] == "UNCHANGED"
    refreshed = body.model_copy(update={"refresh_content": True})
    result = sync._ingest_with(refreshed, store=store, gcs=_Gcs(), drive=Drive(), settings=_Settings())
    assert result["status"] == "GCS_READY"
    assert store.saved[-1].modified_time == modified


def test_postprocessing_records_original_chunks_and_scores():
    hits = [SearchHit("a" + CHUNK_JOINER + "b", 0.1, SearchSource("f1")),
